...
// Функция для инициализации файлов при запуске
...
async function initStorage() {
  ...
  try {
    ...
    // Проверяем/создаем homework.json
    ...
    // Проверяем/создаем last_schedule.json
...
function detectSubjectFromMessage(text) {
  ...
  for (const [alias, subject] of Object.entries(subjectAliases)) {
    ...
    if (lowerText.startsWith(alias.toLowerCase())) {
      ...
      // Убираем название предмета и знаки препинания из начала
...
function getNextDayName(forceMonday = false) {
  ...
  // Если воскресенье и не принудительный понедельник - возвращаем null для автоотправки
  ...
  // Если воскресенье или суббота - следующий день понедельник
...
bot.onText(/\/homework/, async (msg) => {
  ...
  const nextDay = getNextDayName(true); // true = принудительно получить понедельник в воскресенье
...
// Команда для теста отправки в топик (работает всегда, включая воскресенье)
...
bot.onText(/\/start/, (msg) => {
  ...
  bot.sendMessage(chatId,
    ...
    '⏰ <b>Автоматическая отправка в 18:00 (Пн-Сб):</b>\n' +

    '1. Расписание на завтра → топик 3 (с удалением предыдущего)\n' +

    '2. ДЗ по предметам из расписания → топик 2\n\n' +

    'ℹ️ <b>В воскресенье автоотправка отключена!</b>\n' +

    'Но вы можете вручную отправить через команды.\n\n' +

    '🔧 <b>Команды:</b>\n' +

    '/schedule - Отправить расписание на завтра\n' +

    '/homework - Отправить ДЗ на завтра\n' +
    ...
    '/test - Отправить и расписание и ДЗ сразу',
...
console.log('⏰ Расписание и ДЗ будут отправляться каждый день в 18:00 (Пн-Сб)');

console.log('ℹ️ В воскресенье автоотправка отключена. Используйте /test для ручной отправки');
